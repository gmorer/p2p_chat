...
impl Pstream {
    pub fn send(&self, data: Data) {
        match self.state {
            State::Disconnected(x) => {
                console_log!("Cannot send, the client is disconnected since {:?}", x);
                return;
            }
            State::Waiting(x) => {
                console_log!("Cannot send, we are waiting for answer since {}", x);
                return;
            }
            _ => (),
        };
        match (&self.socket, &data) {
            ...
            (Some(Socket::WebRTC(socket)), Data::RtcData(data)) => {
                socket.send(data.as_str().as_bytes())
            }
            _ => console_log!("Invalid data type for websocket"),
        };
    }

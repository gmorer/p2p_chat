...
pub async fn send_static(req: Request<Body>) -> Result<Response<Body>> {
    ...
    let file = match File::open(format!("{}/{}", static_folder, uri)).await {
        Ok(file) => file,
        Err(_) => File::open(format!("{}/{}", static_folder, "index.html"))
            .await
            .expect(&format!("cannot find index.html in {}", static_folder)),
    };
...
async fn main() -> Result<()> {
    ...
    let new_service = make_service_fn(move |conn: &AddrStream| {
        ...
        async move { Ok::<_, hyper::Error>(service_fn(move |req| handler(peers.clone(), addr, req))) }
    });

    let server = Server::bind(
        &addr
            .parse()
            .expect(&format!("Invalid server address: {}", addr)),
    )
    .serve(new_service);

...
use crate::Sender;
use crate::{console_log, slog};
use crossplatform::id::{Axe, Id};
use crossplatform::proto_rtc::{RTCContent, RTCData};
...
struct Peer {
    id: Id,
    socket: RTCSocket, // Id of his connections maybe
}
...
impl<'a> Network<'a> {
    ...
    pub fn insert(&mut self, mut socket: RTCSocket, id: Id, sender: Sender) -> Option<()> {
        ...
        match self.id.get_axe(id) {
            Axe::Top => match &self.top {
                Some(top) => {
                    if self.id.distance(&top.id) > distance {
                        let peer = Some(peer);
                        self.peer_cache
                            .push(std::mem::replace(&mut self.top, peer)?);
                    } else {
            ...
            Axe::Right => match &self.right {
                Some(right) => {
                    if self.id.distance(&right.id) > distance {
                        let peer = Some(peer);
                        self.peer_cache
                            .push(std::mem::replace(&mut self.right, peer)?);
                    } else {
                        self.peer_cache.push(peer)
                    }
                }
            ...
            Axe::Left => match &self.left {
                Some(left) => {
                    if self.id.distance(&left.id) > distance {
                        let peer = Some(peer);
                        self.peer_cache
                            .push(std::mem::replace(&mut self.left, peer)?);
                    } else {
                        self.peer_cache.push(peer)
                    }
                }
        ...
        match &data.content {
            RTCContent::Message(msg) => {
                if let Some(target) = data.to {
                    if target == self.id {
                        self.html
                            .chat_private(data.from.to_name().as_str(), msg.as_str())
                    } else {
                        // self.send_to()
                        self.send(data, from);
                    }
                    ...
                    self.html
                        .chat_msg(data.from.to_name().as_str(), msg.as_str());
                    self.send(data, from);
                }
            }
    ...
    pub fn refresh_html(&self) {
        ...
        self.peer_cache.iter().for_each(|peer| {
            self.html.append(
                ids::CACHE_PEER_ID,
                format!("<span>{}</span>", peer.id.to_name()).as_str(),
            )
        })
    }
    ...
    pub fn remove(&mut self, id: Id) -> Result<(), String> {
        ...
        self.html
            .chat_info(format!("{} as disconnected.", id.to_name()).as_str());
        self.refresh_html();
        Ok(())
    }
